...
**At a glance (38 slices):** ✅ 8 confirmed · 🟢 27 built / ✋ pending · ⛔ 1 revert · groups touched: G05 · G06 · G07 · G08 · G09 · G10 · G11 · G13 · G14 · G16 · G18 · G20 · G25 · G27 · G29 · G30 · G31 · G32
...
|---|---|---|---|
| 2026-07-26 | G10 | **The background strength setting is gone — the mod works it out itself** (§H, "Jar B") — `/cb tolerance` is **deleted**, and so is the per-block strength the Background Studio used to remember. Background handling is now just **Auto or Off**. Auto decides from the picture in five steps, most-certain first: it uses the file's own transparency if it has any, then a background colour it can read for certain, then which area actually wraps the picture's edge, then five different measurements voting on where the background ends — and it **refines every soft edge** by working out how much of each edge pixel is really subject, so edges come out smooth instead of stepped. **If it can't tell, it says so and leaves your picture completely alone** rather than guessing and wrecking it, and tells you to run the new **`/cb bgpick <id> <colour>`** — you name what the background colour is, and it re-bakes straight away from the picture it already has. Name a colour that isn't in the picture and it says so instead of doing nothing. Visible wins: **striped artwork survives** (the old code ate the red stripes off it), **the hole inside a letter finally clears**, and a picture with real transparency or hair-thin outlines comes out **exactly** as before — verified pixel-for-pixel. Old settings still parse: `BgRemove`, `BgRemove&More` and `BgSmart` all just mean Auto now, and a config value the mod doesn't recognise comes up **Auto**, never silently off. ⚠️ On a low-quality JPEG a few specks of background colour can still cling to an edge — that's the deliberate rule protecting thin shading inside your art, not a bug | 🟢 ✋ |
| 2026-07-25 | G10 | **Backgrounds cut cleaner — the colour maths under every bake is fixed** (§H groundwork, "Jar A") — four invisible-but-wrong pieces of maths were replaced, and picture quality visibly improves without anything changing in how you use the mod. **Thin outlines and small text survive colour variants now**: recolouring Tux keeps his whole black outline, and a picture with fine dark text next to the background keeps every letter — the old code ate up to 4 px of any dark art touching the background. **No more dark rim** along soft edges: edge pixels used to be blended with the maths that computer screens use for storage, not the maths light actually adds up in, which painted a dark halo around anything soft-edged on a dark fill; the two patches that existed just to hide that halo are deleted with the cause. **Colour matching is honest across the palette now** (the same "how different is this from the background" budget used to be loose on blues and strict on greys). And the **giant-picture case got ~2× faster** because a whole-image scan the old patches needed is gone. ⚠️ The strength number (`/cb tolerance`) still exists — deleting it and adding the automatic decision is the next jar (“Jar B”) | 🟢 ✋ |
| 2026-07-25 | G11 | **A block can be in more than one category now** (§A) — a block used to hold exactly one category, and putting it somewhere new **silently took it out of where it was**. Now `/cb setcategory <block> shop arabic` puts it in **both**, and none of them counts as the "main" one — put a name in quotes if it has spaces (`"Arabic Letters"`). Nothing is ever left with no category at all: take away the last one and the block lands in the built-in **Uncategorized**, which can't be deleted. New verbs: **`/cb category create`** makes an empty category (categories no longer vanish when the last block leaves), and **`/cb category set` / `remove`** add or drop one category at a time. **Deleting a category now asks what you actually mean:** plain `/cb category delete shop` just takes the category off its blocks and **keeps every block** (what it always did); add **`exclusive`** to also delete the blocks that were *only* in that category — that one asks for **`/cb confirm`** first, shows you the exact list, and **one `/cb undo` brings them all back**; or add **`move <somewhere>`** to re-file the blocks first. `sort` became **`filter`**: `/cb category filter shop newest` orders the blocks inside a category, `/cb category list most` orders the categories themselves (by name, newest, oldest, most blocks, colour, or hiding empty ones). **`/cb category info shop list`** now names every block, and tab-complete offers your **real** category names — including empty ones — so a typo can't quietly invent one. Two names that would collide internally (`Arabic Letters` vs `arabic-letters`) are **refused and told which one already exists**; typing a name in the wrong case still just finds it. `/cb bulkcategory` **adds** now too, to match. ⚠️ **Known:** the old chest category menus, the HUD and exports still show **one** category per block — that's on purpose until the new category windows land in G27 | 🟢 ✋ |
| 2026-07-24 | G30 | **Placed Mask Mode — hide the blocks *you* place** (§H) — one toggle, **`/cb guess placed`**, and every custom block you put down from that moment on shows up as the **"?" cube to everyone else**, while **you** keep seeing your real build. Run it again and the whole thing turns real for everybody at once, and chat tells you how many blocks were revealed. Blocks that were already there, blocks other people place, and every vanilla block are **never** touched — only what you place while it's on. Two people can run it at the same time and stay independent: each sees their own build and "?" for the other's. For everyone else the disguise is complete — the look-at name reads `???`, the selection box is a plain cube so a slab can't give itself away, and breaking one throws **"?" debris**, plays a neutral **stone break sound** instead of the real material, and leaves a **"?" item on the ground** (it turns back into the normal block the moment they pick it up). Your own hand, hotbar and inventory are never disguised — the mask starts at placement. Masked builds **survive relog and a server restart**. ⚠️ Op-level decides who may *run* it, never who may *see through* it — another operator still sees "?" | 🟢 ✋ |
| 2026-07-23 | G09 | **Backups now save everything, and safely** — a backup used to copy only your blocks, textures, and settings, so restoring an old one left your notes, categories, locks and markers in a mixed, half-old state. Now a backup captures your **whole** CustomBlocks folder and a restore brings **all** of it back exactly — and because unchanged files (like textures) are stored only once, you can keep far more backups without filling your disk. Names are cleaner and sortable (`auto_…`, `safety_…`, `manual_…`), and the app can no longer accidentally delete the wrong one. New tools: **`/cb backup preview`** shows what a restore would change *before* you do it, **`/cb backup contents`** lists what's inside, **`/cb backup verify`** checks a backup isn't corrupt, and **`/cb backup load <name> <item>`** restores just one part (e.g. only your notes). Auto-backups can run **at a set time each day** and stop at a **disk budget**; a **pre-update backup** is taken automatically when the mod version changes; and if your data ever goes missing, the server warns you at startup and won't overwrite your good backups. Backups can also be **pulled back from the cloud** by code. ⚠️ The redesigned Backup/Trash **windows** come next (in G27); redeploy the cloud worker before using cloud pull | 🟢 ✋ |
| 2026-07-19 | G05 | **Texture edits always do one clean reload now** (§A · §B scrapped) — the experimental "update the block instantly without reloading" path is **removed**. It could only ever work for some blocks and turned out it couldn't be trusted to show an edit correctly, so every texture change and every world open now goes through **one full resource reload** — the brief, reliable flash you already know. Rapid edits still collapse into a **single** reload instead of a storm, and the earlier corrupt-PNG fix stays in place. Nothing you click changes; edits just always land the safe way | ⛔ revert |
