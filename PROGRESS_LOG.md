...
## G10 §H Stage 1 — golden baselines captured: 49 bakes frozen, diff harness self-tested byte-identical · 2026-07-25 (📝 tooling + baselines, no mod code changed)

The §H rip is staged as **two jars** (owner confirmed this session): Jar A = colour-math correctness with `/cb tolerance` still present (CIEDE2000, linear-light blending, area opening, hysteresis), Jar B = the knob rip + the five-rung cascade + `/cb bgpick`. This session built the Stage 1 safety net both jars are reviewed against.

**`tools/render_preview/BgGoldenPreview.java`** follows the `CheckerPreview` shape: compiles the REAL `BackgroundRemover`/`BgMask`/`CheckerboardDetector`/`ImageProcessor`/`ImageResampler` from `src/main/java` and drives them through the same call chains the mod uses. Seven bakes per picture, one per behaviour the rip touches: `off` (mode none — proves the opaque snap), `edges`/`closed` (tol 30, the config default), `smart` (tol 0, exercising the hidden `smart ? 35 : 0` default), `edges_fill` (CreationCommands' Studio-background rail — 3-arg apply, so the fill only reaches the padding; frozen as-is), `setbg` (BackgroundService.rebake's rail — 4-arg apply + `fillBackground` + `snapBackgroundColor`), and `variant` (recolour rail, `none` forced to `edges`). Red fill `#EE3333` on the colour rails so removal reach is visible at a glance.

**Outputs in `tools/render_preview/bg_golden/`:** `png/` holds the exact baked bytes (the true baseline), `MANIFEST.txt` holds SHA-256 + per-bake wall time (the H19 timing baseline starts here: the 6000×3300 periodic table costs ~9 s per removal bake at tol 30, ~1.2 s with removal off — the cascade must not be slower), `sheets/` + `_ALL.png` are the owner-readable contact sheets. Inputs are the seven TG10 §Setup pictures in `tools/render_preview/bg_in/` (re-downloaded fine).

**Diff mode is proven, not just written.** `java BgGoldenPreview <in> <out> <baselineDir>` re-bakes and compares byte-for-byte, reporting `identical` / `N px moved, max <channel delta>` per bake and writing an 8×-amplified heat map for anything that moved. Self-test: full re-run against the fresh goldens came back 49/49 `identical`, so the pipeline is deterministic and any future diff is a real change.

**Baseline flaws frozen on purpose** (these are what Jar B must beat): stripes picture loses its red stripes (corner-sampled bg = the red stripe), `smart` keeps exactly one white square of it (largest-blob rule), the JPEG chrome logo keeps grey checker residue, the periodic table's title is part-eaten at tol 30 and fully eaten by `smart`, and the letter O's hole survives `edges` (fixed only by `closed`).

**Docs:** TG10 §H Blocked line now says baselines are captured; the golden-capture 📜 bullet left the Planned fold. Health + group check green. Next session: Jar A — CIEDE2000 validated against Sharma's published vectors first, then linear light at every fill-mix site, then re-evaluate `RING_DARK_MAX`/`RING_PASSES`/anti-fringe peel against these goldens.

---

## G11 §A — multi-category membership built: sets replace the one-word field, 3 delete modes, filter, additive setcategory · 2026-07-25 (🟡 built, pending in-game)

Built the whole of G11 §B / TG11 §A from `G11_REWORK_HANDOFF.md` (now marked consumed). Commands only — chest menus keep their single-category view per G11's "commands only until G27". Jar green on JDK 21, every gate passing. Full technical account in [docs/Information/PROGRESS_LOG.md](docs/Information/PROGRESS_LOG.md); the short version:

**A block's categories are now a set** in the new `CategoryMembershipStore`, with `uncategorized` as an undeletable floor a block falls to rather than ever holding zero categories. The key normalizer folds spaces/hyphens/underscores together, which is what lets `arabic letters` resolve `Arabic Letters` while `arabic-letters` is refused as a collision — plain lower-casing could only do one of the two.

**Two owner calls this session.** The legacy `SlotData.category` field stays as a *derived display shadow* of the set (alphabetically-first membership, restamped on every change) so the HUD, Arabic chest menus, exports, blueprint lore, Bulk Workbench and Category Hub stay readable until G27 reworks them — computed, never chosen, never read back as truth, so it is not a stored main. And `/cb bulkcategory` became additive to match `/cb setcategory`, rather than the same word meaning "replace" in bulk and "add" singly.

**Undo needed a new `Kind.CATEGORY`.** Membership isn't on `SlotData`, so a before/after snapshot could only have restored the shadow — undo would have *looked* like it worked while the real memberships stayed changed. Without the new kind, `/cb setcategory` and `/cb bulkcategory` would have silently lost the undo they have today: a regression, not a gap.

**Two things bit during the build.** `SlotManager.createNoSave` takes its monitor then calls into the membership store, so shadowing inside the store's own lock would have closed a deadlock cycle — mutators now shadow after releasing. And the first jar build failed `monolithGate` at `HistoryCommands.java:414`; its pure formatting half moved to `HistoryDescribe.java`, no behaviour change.

**Docs corrected in the same pass:** TG12 §A and TG20 §K still described G11 as carrying "nesting", scrapped 2026-07-25 — both now say multi-membership only and note that exports/shares still carry one category per block. GROUP_27_SCREENS lost its "during migration" line (there is no migration phase) in favour of the membership-store contract. TG11 §A → `Built 🎯`, all A1–A11 cells left at `🎯`. Dashboard regenerated; health and both group checks pass.

---

## TG10/TG14/TG27 — staleness pass: two rows rehomed, cross-group pointers stripped from TG10, implementation notes pulled out of the guides · 2026-07-25 (📝 docs / plan only)

**Two TG10 rows moved to the guide that owns the surface.** The `transparent` refusal row (was C1) is now TG14 §F0 — the refusal exists only because the cutout render layer is unbuilt, so it belongs with the section that will remove it. The bulk tick-selection background row (was C5) is now TG27 §D10 — it drives the `/cb bulk` Blocks List screen; the `setbg` logic itself stays G10. TG10 §C's `Blocked ‼️` flag went with C1 and the section is plain `Built 🎯` again.

**TG10 carries no cross-group pointers any more.** Owner rule: a guide links only to its own group document. Removed the §C "moved out" row, the "restore-then-recolour is G06 §D2" line, the "menus go to G27" setup and cleanup lines, and the TG27 link on the scrapped bulkrecolor bullet (the history text stays). The tolerance duplication between TG10 §E3 and TG27 §Q3 was split by subject instead: G10 owns what a tolerance number does to the fill, G27 §Q owns the panel and its persistence.

**`Partial 🟡` was requested and rejected against the glossary.** `Glossary.md` §2 bans the word `Partial` outright and §11.5 requires the truthful status plus a flag, so §C/§D/§E stay `Built 🎯` despite having confirmed rows. Section ordering already matched glossary §4.

**Stale "moved from" pointers repaired by name, not letter.** TG27 §O/§P/§Q claimed origins of "G10 §G/§F/§B"; G10's group document has since been relettered, so those letters now point at unrelated sections. Rewritten as named scope ("G10's Advanced Recolor Hub scope") so a future relettering cannot rot them again. Same treatment applied to the TG14 §F0 and TG27 §D10 relocation notes.

**Implementation detail pulled out of the guides per glossary §7/§9.5**, preserved here and in the group documents:
- *TG14 §F:* no cutout registration exists for slot blocks and `SlotBlock` is not non-opaque; `OffAtlasImage` flattens alpha onto black on purpose, which a real transparent option must account for rather than fight. (Also already in `GROUP_14_ANIMATION_VIDEO.md`.)
- *TG27 §D:* the ripped natural-language bar was `BulkNlParser` / `BulkNlBar`, mis-targeting because a bare category word fell through to the "all" branch. (Full account in the 2026-07-20 entry below.)
- *TG27 §N:* the chest menus to retire are `BackupMenu`, `BackupConfirmMenu`, `BackupSelection`, `TrashMenu`, `TrashEntryMenu`, plus a repointed `SafetyMenu`; the proposed Trash Screen is `GuiMode` 19. (Also in `GROUP_27_SCREENS.md` §F.)
- *TG27 §O:* the live single hue-shift op that must not be collided with is `BulkRecolorCommands.java`, owned by G07.
- *TG27 §P:* `gui/chest/ColorVariantsMenu.java` exists today as a passive listing-only chest menu — the starting point to replace, not a finished builder.
- *TG27 §Q:* `BgStudioMenu.java` / `BgStudioSession.java` were audited for TODO/placeholder/disabled-button patterns and none were found; no partially wired modes.

**TG10 progress recomputed per the glossary's row definition** (confirmed runnable slots ÷ total runnable slots) instead of the section estimate it had: 25 confirmed rows folded in the archive against 7 still open = 78%, replacing 43%. Cleanup list rewritten to the blocks the remaining rows actually need. `testing_tools.py health` green; dashboard regenerated.

## G10 §F/§G + G06 §D2 — the E3 "regression" was the tolerance scale read backwards; the real picture loss was undo-of-delete, not restore · 2026-07-25 (🟡 built, pending in-game)

**E3 is not a defect, and no code changed for it.** The owner ran `/cb tolerance 10` expecting a generous setting and reported that a colour variant only repainted part of the blue background. `BackgroundRemover` maps the player-facing 0–100 strength linearly onto CIE-LAB ΔE with `MAX_DELTA_E = 22.0`, so **10 is ΔE 2.2 — the second-strictest setting the command can express**, not the loosest. Nothing was broken; the mod matched only blue that was almost pixel-identical to the sampled corner, and JPEG ringing around the white "O" plus the site's watermark are not that identical. Verified offline by running the exact variant rail (`recolorBackground → toBlockPng → fillBackground`, mode `closed`, fill `#EE3333`, 512px) on the same dreamstime source at 10 / 30 / 60 / 100: leftover blue pixels 5967 / 1502 / 0 / 0. At the default 30 the counter of the O fills and only the watermark survives; at 60 nothing blue is left. E2 (chrome shading) and E4 (hair-thin lines) were never at risk because no tolerance, gate or stage was touched. The E3 row now runs at 30 and the guide's 💔 fold is empty. What the owner *does* want reopened is the setting itself — the number reading backwards is real, and the owner's call is to consider removing `/cb tolerance` entirely in favour of automatic behaviour, so that is now a `Discussion ✏️` on §E and a Planned row, not a fix. The reported "hard rectangle of old blue in the lower right" did not reproduce at any tolerance and is still unexplained; the leftover blue here is a speckle ring plus the watermark, spread over the middle.

**G06 §D2: the trash restore rail was innocent; `/cb undo` after a delete was eating the source.** `SlotManager.delete` copies the block into the trash *before* `TextureStore.delete` erases `slot_N.src`/`slot_N.url`, and `TrashCommands.doRestore` writes both back — that path is correct and was never the bug. The undo path is not: `DELETE` ops carry `texture` only, so `applyInverse` restored the slot and its baked pixels while the source stayed erased. The block came back looking perfect and every source-driven command (`/cb setbg`, `/cb resize`, `/cb variants`) then refused it with the honest "no stored picture to re-bake" skip — the exact symptom reported against restore, one step upstream. Delete the same block again afterwards and the trash copy is taken from a block that no longer has a source, which is how a genuine restore ends up empty-handed too. Fixed by `DeletionService.healSourceFromTrash`, called from the `DELETE` undo branch: it reads the source (and the link) back out of the trash copy taken by that same delete, filling gaps only — an existing source is never overwritten, and a block that genuinely has none (Arabic glyphs, video frames) is a silent no-op. Two smaller holes on the same rail closed with it: `TrashManager.capture` now also stores `url.txt` (the link was lost on every delete/restore round trip), and `doRestore` says out loud when an entry has no stored picture instead of leaving the player to discover it three commands later. G10's skip message was not touched — an honest skip on a source-less block is correct behaviour and stays.

**`/cb setbg` on a trashed id no longer dead-ends.** The no-match branch in `BackgroundCommands` now checks the trash before reporting "No block or selection matched", and names the block's deletion time plus `/cb trash`. It points at the trash *screen*, not at "`/cb trash restore`" as TG10 C9 originally worded it, because that command does not exist — `/cb trash` only opens the browser and Restore is its per-entry button. C9's expected text was corrected to match the working action.

**Two chat notices deleted, behaviour untouched.** Owner-locked: image creation prints only the fetching line and the result line. `CreationCommands.FLAT_CHECKER_NOTE` and `ImageProcessor.smallSourceNote` are gone, along with both call sites in create and retexture. `CheckerboardDetector.flattenToBlack` still flattens a preview grid to black inside `BackgroundRemover`, and a small picture is still enlarged by `toBlockPng`; only the two lines went. `isFlattened()` is left in place with no caller and its header note updated.

**Resampling now sharpens in both directions.** A shrunk picture got no sharpen at all (`if (scale > 1.0)`), and the enlarge sharpen was mostly cancelled by `clampLocal` pinning every sharpened pixel back inside its own 3×3 min/max — which is exactly the soft ramp that needs the sharpening. Both directions therefore baked mushy, matching the owner's report on a 447px source enlarged and a 954×1484 source reduced. `unsharpMask` takes an `overshoot` argument: the clamp widens by `overshoot × (local max − local min)`, so the allowance is proportional to real local contrast and is exactly **zero on flat colour**, which is what makes a halo impossible there rather than merely unlikely. `ImageProcessor` applies 0.60 enlarging, 0.35 shrinking, overshoot 0.20, and skips sharpening entirely when no resize ran. Constants were chosen from `tools/render_preview/ResizePreview.java` sheets (the 447px source, Tux enlarged ×1.63, the chrome logo shrunk ×0.64, a 960×528 table shrunk ×0.53) and owner-approved as setting "A" over a stronger "B" that speckled Tux's near-flat white belly. The shipped `toBlockPng` was then diffed against the approved preview: identical on every fully-opaque pixel across all four sources.

`gradlew build` green (`HistoryCommands` needed the heal helper moved into `DeletionService` to stay under the §9.3 400-line handler cap). `testing_tools.py health` reports two pre-existing warnings in TG11 §C, unrelated to this pass and present before it.

## G11 — Category ownership split into the groups that actually own the surfaces; Bulk Retexture removed rather than deferred again · 2026-07-25 (📝 docs / plan only + one tile removal)

TG11 had accumulated work it did not own. Three Screen sections (`CategoryHubScreen`, the Create-workspace Category tab, the Export Dashboard Screen) moved to G27 §N/§R/§S/§T, the export ZIP/download-link row moved to G12 §A as A8, and the unbuilt category vault share/import moved to G20 §K — each a full relocation following the G09→G27 and G10→G27 precedent: content moved with a dated `Moved from G11` note, Ownership/Cross-Group-Contract/Locked-Decision/Superseded rows written on both sides, and the vacated letter closed up in TG11 rather than left as a stub. TG11's own sections were then relettered to run A/B/C in the glossary's priority order.

**Bulk Retexture is now removed, not deferred for a fourth time.** The `CategoryEditMenu` slot-19 tile pointed at `/cb bulkretexture`, which has no command registration and never had one — the log flagged that gap on 2026-06-14 and it survived two rounds of "build or remove" as a `Planned 📜` + `Blocked ‼️` row. Owner decision this session is that it should not exist at all, so the tile and its `promptCommand` handler are deleted (`m.fill()` already backgrounds the slot, so no reflow), the javadoc no longer lists it, and G11 records it as scrapped in Locked Decisions, Superseded Decisions, and the TG11 archive — the Deferred-Scope row naming G10/G07 as a future owner is gone, so nothing points at a revival path. `promptCommand` itself stays: seven other menus use it. `compileJava` green.

**The §A rework then lost about half its surface area in one discussion pass, which is the useful outcome.** Presented as seven dependency-ordered slices it read as overwhelming, and most of it turned out to be unwanted rather than merely unscheduled: hidden/locked/permission-gated categories, per-category sounds and particles, accent colour, per-block badges, and auto-add rules are all scrapped as clutter, so the category record is down to display name, key, parent, icon, colour tag, description, and order. Nesting became a two-level hard data cap instead of an unlimited tree with a warning, a parent now lists its own blocks and merely names its sub-categories instead of merging them (so `blocks in this category` has exactly one meaning), and deleting a parent that still has children is refused outright — which in turn made the whole-tree delete mode unreachable, dropping delete from four modes to three. Main category is explicitly set, never inferred from join order or depth. Names stop being forced lowercase: a category stores the display name as typed plus a lowercase key for matching, so `Arabic Letters` finally displays like the rest of the project. The owner declined the variant that also freezes the key, so renaming still changes identity for anything holding an old name — recorded rather than quietly assumed away. Four old 2026-06-30 decisions moved to Superseded rather than being edited in place.

**Migration stopped being a phase.** The plan had a gated migration — backup, dry-run, confirm — as the mandatory first slice, on the reasoning that turning a category word stored per-block into category records with membership lists is the one step that can lose assignments. The owner's answer was that nothing is stored in categories at all, which removes the data the gating existed to protect. What replaces it is a first-load conversion that runs only if a legacy category word is actually found and is a no-op otherwise: a handful of lines, no gate on the rest of the model, and stray assignments still convert instead of silently reading as uncategorized. `/cb category create` now makes empty categories while today's assign-and-it-appears habit keeps working.

**One stale cross-reference class got fixed while it was cheap.** The relocation notes cited source letters (`G11 §B`, `G11 §D`, `TG11 §A`) that the same relocation had just vacated or reassigned — TG27 §T was telling readers the export schema lived at TG11 §A, which by then was the retexture tile. Provenance notes are now letter-free (`Moved from G11`) and the one real pointer aims at TG12 §A where the artifact actually lives. `testing_tools.py health` passes on every guide; dashboard regenerated.

## G10 — Every base block was shipping a texture with alpha onto an alpha-tested layer; "closed areas" was a colour key, not a region test · 2026-07-25 (🟡 built, pending in-game)

Started from an owner screenshot of a glitched coloured hairline tracing a block at point-blank range, with the note that the block's own colour variants were clean. Four defects came out of it, all measured against real sources offline before anything was changed, plus §C and §F3 built on top.

**The outline: the base bake rail could hand the atlas a texture with alpha; the variant rail could not.** `BackgroundRemover.snapBackgroundColor` — the last step of every non-recolour bake — opened with `if (NONE.equals(mode) || tolerance <= 0) return png;`. With the shipped default `backgroundMode = "none"`, that is an immediate return, so nothing ever flattened the alpha that `ImageProcessor.toBlockPng` introduces as letterbox padding around a non-square picture, or that a transparent PNG carries in the first place. Slot blocks draw on `RenderLayer.getEntityCutoutNoCull` — alpha-tested, mipmapped — where a semi-transparent texel passes or fails the test differently per mip level and per angle, which is exactly the flickering coloured hairline that was reported. The colour-variant rail never showed it because it always ends in `ImageProcessor.fillBackground`, which composites everything onto an opaque fill. Measured through the real pipeline: Tux at mode `none` came out with 6098 partial-alpha texels and 144714 fully transparent ones; the same source at mode `edges`/tol 30 came out fully opaque, which is why the bug looked intermittent. The snap step now always composites leftover alpha onto the fill and only the *snap* stays gated on mode/tolerance, so an opaque source is still byte-identical while a source with alpha can no longer reach the atlas. This is also what the client already wanted: `OffAtlasImage` flattens alpha onto black for off-atlas blocks with the comment "owner wants them BLACK by default" — atlas blocks simply had no equivalent, which is why only some blocks showed it.

**"Background + closed areas" was eating artwork because it was a colour key with no connectivity.** Stage 1b was a flat per-pixel loop: any pixel anywhere in the image within ΔE of the background colour became background. On a logo composited onto a dark background, every dark shading line inside the chrome matches that dark background, so the repainted colour showed straight through the subject — reproduced exactly, green chewing into the Chevrolet bowtie's chrome. The fix is to treat an enclosed area as a *region*: candidates are grouped into connected components, and a component is absorbed only if it is thick enough to hold a small disk (a city-block distance transform, so the whole test stays O(w·h) rather than repeated erosions per component). The counter of a letter "O" is tens of pixels thick and still fills; a 3 px shading line does not. Verified both directions on controls — the ring's hole fills with the variant colour, and a chrome plate's 3 px shadow lines survive at the same mode and tolerance.

**Thin detail was disappearing into the morphological close.** `BgMask.morphClose` dilates the background mask then erodes it, which is a no-op for subject features 3 px and wider but is not reversible below that: the dilate joins the background across a 1-2 px stroke and the erode cannot reopen it, because the closed-over pixels no longer have a non-background neighbour. That is a plausible cause of the recurring "creating an image eats its edges" reports. The close is now colour-guarded — a pixel plainly far from the background colour (ΔE beyond `max(2·tol, 15)`) is never swallowed by morphology, while near-tolerance pixels, the ones the close exists to bridge, stay unguarded so the original hairline-gap fix is untouched. A control of 1 px and 2 px strokes now survives the bake intact.

**Checkerboard detection died on a re-compressed copy of an image it already handled.** The owner's `/cb variants chevrolet …` used a Google-image JPEG thumbnail and came out checkered, while the site's own PNG of the same logo worked. Measuring the border histograms explains it: the PNG puts 50 % of the border in bucket 6 and 50 % in bucket 10, whereas the JPEG smears the same two tones across buckets 4-11, leaving the peaks at 24.5 % and 18.6 % — under the 20 % share each tone had to clear. Worse, "the two biggest buckets" could return one tone and its own ringing. Tone share is now measured over all in-regime border mass split at the two peaks (the same nearest-tone classification the periodicity test already used), and the second peak must sit at least two buckets from the first. Neutrality, the L* gap and the two-axis periodicity test are unchanged, so the striped-artwork controls added on 07-24 still pass; the JPEG thumbnail now flattens to black.

**§C built, scoped to what can actually render.** `SlotData.background` stores `black` / `#RRGGBB` / `transparent`, persists (omitted from JSON when it is the default, so existing files are untouched), and re-bakes through one `BackgroundService` rail that every surface calls. Black deliberately keeps the exact existing chain so setting a block to black is a no-op re-bake rather than a subtle re-render. `/cb setbg` (alias `/cb setbackground`) takes one id or the bulk scope grammar, holds all-block changes behind `/cb confirm`, and records a RETEXTURE undo op — the one Kind that already restores both the slot snapshot and the pixels, so one `/cb undo` reverts the choice and the picture together. `transparent` parses and stores but is refused at the command with an honest message: no cutout registration exists for slot blocks and `SlotBlock` is not non-opaque, and that registration is G14 §E's half of the contract. Building it anyway would have re-created the very hairline artifact the opaque-bake rule above exists to remove. Noted in G14 and TG14 as the sole remaining blocker.

**§F3 built.** `/cb colorvariants delete <id>` removes base plus whichever colours exist, behind `/cb confirm`, as one undo batch, reusing `ColorFamilyOps.takenTargets` for membership and `DeletionService.deleteCore` for the deletion itself — so a family delete frees slots and swaps placed copies to "Deleted: &lt;name&gt;" markers identically to `/cb delete`, instead of leaving purple blocks.

**Two size gates tripped on the way and were resolved as real splits, not trims.** `SlotManager`'s eight attribute setters were byte-identical apart from their `withX` call and collapsed onto one `mutate` rail; `BackgroundRemover`'s fringe peel and ring absorb moved into `BgMask`, which exists for exactly that kind of pure grid operation. Both are behaviour-neutral, and that was checked rather than assumed: the 312 harness output images are byte-identical before and after the refactor.

## G10 — The colour-family size gate was guarding the download, not the image; the confirm hold was dropping actions silently · 2026-07-24 (🟡 built, pending in-game)

Started from two owner reports — §D "still regressed, checkerboard in game" and a "bad outline after uploading" — and turned into three sweeps of the whole colouring/background subsystem. Four outcomes: two defects fixed, one diagnosed but deliberately not fixed, one report ruled out as not a bug at all.

**The size gate was attached to the fetch, not to the image.** `ImageLimits.downloadColorFamilySource` validates as part of downloading, so every rail that obtains bytes some other way skips it entirely. `ColorFamilyOps.rebuildFamily` is exactly that rail: it reads `TextureStore.loadSource(index)` and bakes three textures from it without ever passing a limit. That is not theoretical, because the plain create path downloads through `ImageDownloader.DEFAULT_MAX_BYTES` — 25 MB and **no pixel cap at all** — so a 6000×3300 source can be stored legitimately by `/cb create` and then handed to a rail whose own contract caps it at 4096 px / 12 MP. `requireColorFamilySource` is now public, gained the body check the downloader used to provide implicitly, and runs on the stored bytes before `recolourSet`. Dimensions still come from `ImageProcessor.dimensions`, a header-only `ImageReader.getWidth(0)` read, so an oversized source is refused before anything decodes it into heap — the standard pixel-flood defence, which the code already had and simply was not reaching.

**What was deliberately left alone.** `/cb resize` and the bgstudio apply have the same shape — both re-render from a stored source with no family limit — and both were left untouched on purpose. They belong to the create path's looser contract, so tightening them to the family's 4096 px would start failing blocks that resize correctly today. That asymmetry is now written into the G10 Technical Contract rather than silently "fixed" into a regression.

**One pending slot, no announcement.** `BulkConfirm` holds exactly one action per actor and its `request()` did a bare `PENDING.put`, discarding any earlier hold without a word. Queue "overwrite family A", queue "overwrite family B" before confirming, type `/cb confirm`, and B is overwritten while the chat log still shows A's prompt — the one flow whose entire purpose is that a destructive batch never surprises anyone. `request()` now says when it displaces a live hold, and `confirm()` names the action before running it. This is shared G07 infrastructure, so every bulk op inherits both lines.

**Dark checkerboards: diagnosed, not fixed.** Every gate in `CheckerboardDetector.detect` requires a tone at or above `LIGHT_MIN = 190` — `tallyBorder` refuses to histogram anything darker, `alternations` cannot see the periodic switches, `sampleTone` cannot average the tones — and `BORDER_LIGHT_FRAC` then bails when the light count misses 85 % of the border ring. The owner's citypng source measures 25/25/25 against 40/40/40, so `lightCount` is literally zero, `detect()` returns null, and the grid bakes onto the block. Confirmed by pulling that PNG and measuring it, not inferred. The widened rule has to admit dark two-tone grids without eating genuinely dark artwork, so it stays unbuilt behind the preview-approval pass rather than being guessed at.

**The "bad outline" was not a bug.** `HorseNew.png` is 900×1000 with pure 0/255 alpha — no anti-aliasing anywhere in the source. The reported block is the family's base, and the base rail never touches interior pixels (the ring absorb in `BackgroundRemover` runs only when `forcedFill` is set), while the anti-fringe peel is skipped outright for a transparent background. Nothing in the mod modified those pixels: the hard black squares at the eye and hoof are the source's own unaliased pixels seen at point-blank zoom. Recorded in TG10 as do-not-re-investigate so the next session does not spend another pass on it.

**Dark checkerboards, after approval: two regimes, and a stripe bug found on the way.** The widening itself is small — `detect()` now runs its signature once for a near-white tone pair and once for a near-black one (`DARK_MAX = 96`, comfortably above the measured 25/40 and far below any mid-tone content), with a shared `inRegime` gate so `tallyBorder`, `alternations` and `sampleTone` cannot drift apart. Everything that makes the test strict is applied inside whichever regime is being tried, so the grid still has to be neutral, split two tones across the border ring, sit in the same small L* gap, and repeat.

Previewing it against controls is what caught the second defect. Dark striped ARTWORK was flattened — and so was light striped artwork, which proved the flaw predated the dark work rather than being caused by it: `detect()` took `max(altTop, altLeft)`, so alternation on either axis alone satisfied periodicity, and stripes alternate on one. A checkerboard is periodic in two dimensions, so that is now `min` — both axes must switch. Stripes survive as the design they are, in both regimes, and all four real/synthetic checkerboards still flatten.

Verified through the real class, not a reimplementation: `tools/render_preview/CheckerPreview.java` compiles `CheckerboardDetector` straight from `src/main/java` and calls its public entry points, writing a before/after sheet per source. Ten cases pass — light standard, dark standard, dark at another cell size, the owner's citypng chevrolet, and six controls (solid black background, dark gradient, dark stripes, light stripes, the real transparent-PNG demo, the horse) all correctly untouched. The chevrolet's chrome edging and the dotted fill inside the bowtie survive intact.

**Also fixed while in TG10:** three of the five §D retest links were dead (two returned HTML, one a 404 from a bad path guess of mine). Every link in that table now resolves to a real image, checked with a request rather than assumed.

**Files:** `image/CheckerboardDetector.java` (`DARK_MAX` + `inRegime` + per-regime `detectRegime`, both-axis periodicity, header rewritten), `image/ImageLimits.java` (validator made public, body check, stored-source doc), `command/ColorFamilyOps.java` (gate the rebuild rail), `command/handlers/BulkConfirm.java` (announce replacement + confirmation), new `tools/render_preview/CheckerPreview.java`. Docs: TG10 verdict/§D/D5/D6/F5/F6 + repaired retest links, G10 locked decisions + technical contract + §C requirements, Dashboard, this log.

**Next:** owner runs TG10 D5 (citypng chevrolet flattens), D6 (striped source survives), F5 and F6 on `customblocks-1.0.0.jar`.

## G30 §H — Placed Mask Mode: `/cb guess placed` hides the blocks you place from everyone else · 2026-07-24 (🟡 built, pending in-game)

§H is the inverse of §A. §A blinds one flagged holder while watchers keep the truth; §H blinds every watcher while one runner keeps the truth. One toggle, `/cb guess placed`: every CustomBlock the runner places while it is on shows as the bundled QuestionMark to everybody else, the runner keeps seeing their real build, and toggling off reveals the whole thing at once.

**The mask is a record, not a rule.** Everything hangs off the locked decision that masking is decided per placement and never recomputed from block type. `PlacedMaskStore` holds, per runner, the set of packed `BlockPos` keyed by dimension; `SlotBlock.onPlaced` records, `onStateReplaced` releases. That is what makes it exactly reversible — blocks placed before the toggle, blocks placed by anyone else, and every vanilla block simply never got a record, so they can never be revealed or hidden by accident. It also means the store is mutated per BLOCK, not per command, so unlike `GuessModeStore` its disk write is debounced onto the server tick (5 s) with an explicit flush on toggle and on `SERVER_STOPPING`.

**Who may see through it is decided on the server, not the client.** `PlacedMaskStore.feedFor(viewer)` returns every runner's positions EXCEPT the viewer's own, and that is the packet each client gets. So the runner's client is not asked to hide anything from itself and a second operator is not accidentally privileged: op level gates who may RUN the command, never who may SEE through a mask. Two runners are independent for free — each is excluded only from their own set.

**Deltas, because a build is not a command.** Re-broadcasting the whole position set on every placement would be quadratic; a 5,000-block build would re-send 5,000 positions 5,000 times. `PlacedMaskPayload` is a compact binary payload (8 bytes a position, not ~20 characters of JSON) with three ops: FULL on join and on toggle, ADD to everyone-but-the-runner per placement, DEL to everyone per break. It rides `GuessSync` as the Group doc requires — the same route, its own packet. Client-side `ClientPlacedMask` keeps fastutil `LongOpenHashSet`s per dimension behind a single volatile `EMPTY` flag, so on a server where nobody runs the mode the whole feature costs one boolean read per render call, and the current dimension's set is memoised against the STATE it was built from (so a feed update expires the memo instead of handing a renderer a stale set).

**The leaks, and the seam each one closes.** The block itself is drawn by `AnimSlotBER`, which already knows how to paint the inflated "?" cube over a real block for §A — §H reuses it verbatim, fixed to `NO_LOOK` since §H never consults §B's look chain. `GuessDisguise.blindsAt(pos, state)` is the position-aware version of the existing `blinds(state)` gate, so the outline mixin (a slab must not give itself away by its selection box) and the particle mixin (crack and debris sample the REAL texture) now serve both directions from one predicate. The look-at HUD blanks id and name the same way it already does for a flagged holder.

**Break sound.** Verified against the 1.21.1 bytecode rather than assumed: `Block.spawnBreakParticles` emits `syncWorldEvent(player, 2001, …)`, and BOTH the breaker (locally, via `ClientWorld.syncWorldEvent`) and every observer (via the world-event packet) land in the same `WorldRenderer.processWorldEvent(int, BlockPos, int)`. One mixin there covers everyone: for a masked position the vanilla effect is cancelled and replaced with a plain stone break sound plus the existing "?" debris.

**The drop.** A masked block still drops the real item, which would walk the answer out of the mask as a stack lying on the floor. `SlotBlock.getDroppedStacks` stamps it with the runner's uuid in `custom_data` (invisible — vanilla never renders that component), and the item render seam draws "?" for anyone who is not that runner. Two details make it behave: the disguise is limited to the GROUND and FIXED transformation modes, so a tagged stack can never disguise a hotbar or inventory slot; and `PlayerInventory.insertStack` strips the tag on RETURN, only on success, so a pickup that FAILS (full inventory) leaves the drop masked instead of quietly revealing it, a partial pickup leaves the remainder on the ground masked, and what lands in the inventory is byte-identical to a normal stack and stacks normally. Ordering was the subtle part: Minecraft removes the block (→ release) BEFORE it rolls the drops, so `PlacedMaskStore` remembers the position it released for exactly one step, matched on dimension AND position so a stale value can never tag the wrong item.

**One removal that is not a removal.** `onStateReplaced` fires whenever the block at a position changes, including when G13's Arabic join flow swaps a letter for a different FORM of the same letter in place. Releasing there would have made a masked Arabic word flash its real letters at every watcher the moment the word re-flowed. The release is therefore skipped when the replacement is itself a `SlotBlock` — a swap, not a removal, so the position keeps its mask. A Deleted marker is not a `SlotBlock`, so wiping a block definition still lifts the mask correctly.

**Accepted limits, recorded rather than papered over.** The mask is painted by the block-entity renderer, so it stops at the same render distance every off-atlas custom block already stops at — this is a pre-existing property of the mod's placed rendering, not something §H introduces, and extending it only for masked blocks would have made thousands of forced BER draws. And only the BREAK sound is neutralised; the repeated hit sound while mining still comes from the real material, the same gap §A left when it deferred sound work. Both are now written into the §H Boundary.

**Files:** new `core/PlacedMaskStore.java`, `core/PlacedMaskHooks.java`, `core/PlacedMaskTag.java`, `client/ClientPlacedMask.java`, `network/payloads/PlacedMaskPayload.java`, `command/handlers/PlacedMaskCommands.java`, `mixin/PlacedMaskSoundMixin.java`, `mixin/PlacedMaskPickupMixin.java`. Edited `network/GuessSync.java` (§H send/broadcast/delta), `block/SlotBlock.java` (record / release / stamp), `client/render/GuessDisguise.java` (`maskedWorld` / `blindsAt` / `maskedDrop`), `client/render/AnimSlotBER.java`, `client/HudRenderer.java`, `mixin/ParticleDisguiseMixin.java`, `mixin/AbstractBlockStateMixin.java`, `mixin/ItemDisguiseMixin.java`, `command/handlers/GuessCommands.java` (node + status readout), `PayloadRegistrar.java`, `CustomBlocksMod.java` (tick + flush), `client/CustomBlocksClient.java` (receiver + disconnect clear), `customblocks.mixins.json`. Docs: G30 §H requirements/boundary/technical contract, TG30 verdict + §H → Built 🎯, this log.

**Next:** owner runs TG30 §H with two clients (three for H5) — H1 toggle on, H3 toggle off, H5 two runners, H6 look-at HUD, H7 break + pick up, H8 restart.

## G34 — Wheel face is now ONE baked texture, not 850 quads · 2026-07-24 (🟡 built, pending in-game)

Third in-game rejection of the same face: still stair-stepped, "not perfect and seamless". Screenshot shows a staircase down every wedge boundary whose steps DOUBLE outward — the exact profile of the strip stacks' overhang, i.e. the overhang is not being covered.

**Why the strip design cannot work in Minecraft, though it works in the mockup.** The geometry is sound: strip `k` of slice `s` spans heights `[y0, y1]` from the slice's own boundary ray and starts at `x0 = y0·cot(SLICE_DEG)`, so for `y > y0` it juts past the ray into slice `s+1`'s territory — the intended overhang, to be painted over by `s+1`, which carries a higher depth. The Python mockup renders that correctly because it draws in strict depth order: painter's algorithm, guaranteed. Minecraft gives no such guarantee. The strips are translucent TEXT_DISPLAY **background** quads separated by 0.002 blocks, drawn through the entity translucent path, and that ordering does not hold — so the overhangs surface instead of being buried. Every earlier "fix" was tuning the shape of an overhang that was never going to be reliably covered. Two builds, three rejections; the premise was the bug.

**The fix: no assembly at all.** The disc is now a baked 1024² RGBA texture, `assets/customblocks/textures/block/wheel_face.png`, generated by `tools/gen_wheel_face.py` and worn by a single ITEM_DISPLAY. A raster has no ordering to lose: the generator assigns every texel to exactly one slice, so a wedge boundary is a true straight ray and there is nothing left to overlap, z-fight or tear. 850 entities → **1**.

**Generator details.** Colour comes from `floor(angle / SLICE_DEG + 0.5) mod SLICES` — literally the rule `WheelRing.sliceAt` uses to name the winner, so the colour the arrow stops over is the slice the code awards, by construction (`floor(x+0.5)`, not numpy's `round`, which breaks ties to even). Pixels OUTSIDE the disc are coloured too and only alpha varies with radius: a half-covered rim texel then blends the right hue at half alpha instead of fading toward black. 8×8 box supersampling makes every boundary a one-texel colour ramp — ~2 cm on the placed 20-block wheel — rather than a step. Palette unchanged: 50 golden-angle hues at S=V=1.

**Model, verified against 1.21.1 bytecode rather than assumed.** `models/block/wheel_face.json` is a flat double-sided quad with no parent, so every `ModelTransformationMode` is identity; `ItemRenderer.renderItem` applies `translate(-0.5,-0.5,-0.5)` after that, so the model's `[0,16]` box maps to `[-0.5,+0.5]` blocks and the disc lands centred on the anchor. Scale inverts the baked margin exactly: `FACE_SCALE = 2·OUTER_R · TEX/(TEX − 2·MARGIN)`, putting the painted rim on `OUTER_R = 10`. Frustum culling checked too — `DisplayEntity.updateVisibilityBoundingBox` sets `ignoreCameraFrustum` when `width` or `height` is 0 (the default), so a 20-block quad on a 1-block entity is never culled off-screen.

**Backside, per owner request.** The back face's UVs are U-flipped to `[16,0,0,16]`. Derived from `CubeFace`/`ModelElementTexture` bytecode: corner 0 of NORTH is `(maxX, maxY)` and takes `(u1,v1)`, so the DEFAULT north UVs would read un-mirrored from behind — wrong here, because the icon ring's positions are physical and a viewer behind the wheel sees them mirrored. Flipping makes the face mirror with them, so wedge and icon still line up slice for slice from either side.

**Files:** `wheel/WheelRing.java` (strip constants, band table, palette and `spawnWedges`/`stripNbt` all deleted; `FACE_*` + `spawnFace`), `wheel/WheelDisplayVisual.java` (`Handles.wedges` list → single `face` UUID; dead `applyQuadColor` + `BG_*` removed), `wheel/WheelBlockEntity.java` (NBT `wedges` → `face`, older saves rebuild via the tag sweep), `wheel/WheelBlockRegistry.java` (render-only `wheel_face` item), new `tools/gen_wheel_face.py`, `models/block|item/wheel_face.json`, `textures/block/wheel_face.png`. Docs: G34 locked decisions + technical contract + superseded rows, TG34 verdict + §A pass rule + new rows A11/A12 (back side), this log.

**Next:** owner re-places the wheel and checks TG34 §A — A9 (edge is one straight line), A10 (no hub flicker), A11/A12 (back side shows, mirrored, icons still on their own wedge).

## G34 — Wedge face rebuilt: exact strip stacks, straight edges meeting at the true centre · 2026-07-24 (🟡 built, pending in-game)

With the dots fixed (entry below), the owner could finally see the face and rejected it: stair-stepped wedge edges, and colours twitching around the hub. Two independent bugs.

**Steps.** Each wedge was 12 rectangles stacked ACROSS the radius, each sized at its widest end, so every band overhung the true sector lower down — 12 visible steps per edge. **Twitch.** The depth stagger was `(index % 6) * 0.004`: 600 quads sharing 6 depth levels, so at the hub, where every wedge overlaps, different-coloured quads tied on depth and z-fought.

**The fix — stop trying to taper a rectangle.** A rectangle can never taper, so no stack of them can BE a pie sector. But a wedge does not need two exact edges; it needs ONE. A slice is now built as strips running PARALLEL to its own boundary ray, started on that ray and left deliberately oversized on the far side. Slice `s+1` is painted on top and its own exact edge shears the overhang off. Every visible boundary in the wheel is therefore some slice's exact edge — a straight ray through the origin — so all 50 wedges converge on the true centre with no cap and no steps. Strip heights DOUBLE outward, which reaches the apex in 17 strips instead of thousands; ratio 2 is a hard ceiling, because at exactly 2 a strip's far corner reaches 14.18°, still inside slice `s+1` which covers it — any higher and the overhang escapes onto already-painted slices and the seam tears. Depth is now one distinct level per slice (50, monotonic), so two different colours can never tie.

**Wrap-around.** Slice 49's overhang would land on slice 0, already painted. So slice 49 alone is built two-sided: a mirrored half-stack from each of its edges, the two ragged sides meeting on its own bisector where they are colour-on-colour and invisible.

**Two more leaks the mockup's pixel sweep caught, which eyeballing had missed.** A sky-counting detector over the rendered face found 721 stray sky pixels inside the disc and 1889 at the rim. (1) Strips were clipped against their TOP edge, so each fell short of the circle lower down — now clipped against the bottom edge plus a 0.02 overshoot, so the scalloped rim lies wholly outside `OUTER_R`. (2) Strips abutted exactly, and abutting-but-unshared edges crack at the T-junction — each strip's BOTTOM edge now drops halfway into the strip below (same colour, already covered); dropping the bottom and never raising the top leaves the overhang angle untouched, so the exact-edge property survives. Both now sweep clean: the residual count stays flat as render resolution rises while the disc area quadruples, which is the signature of rasteriser noise, not holes.

**Ported under verification, not by eye.** A checker replicates the new Java math and compares it to the approved mockup corner-for-corner: worst deviation **1.6e-14 blocks**. It also guards the trap in the port — the mockup put slice `s` in `[s·7.2°, (s+1)·7.2°]` while icons sit at `s·7.2°`, so a naive port would have centred every icon on a wedge boundary; the stack is built off `sliceCenterDeg ∓ SLICE_DEG/2`, and the checker asserts all 50 icons sit mid-wedge and that `sliceAt()` still returns the slice actually under the arrow.

**Numbers:** 861 wedge quads (was 600); centre convergence error 0.0008 blocks (~1/1250 of a block, under one texel of a 512px pack); max wedge depth 0.098, still clear of the icon layer at 0.180.

**Files:** `wheel/WheelRing.java` (strip constants + band table + `spawnWedges`/`stripNbt` replacing the ring build). Docs: TG34 verdict + rows A6/A9/A10, this log.

**Next:** owner re-places the wheel and checks TG34 §A — A6 (converge on one point), A9 (edge is one straight line), A10 (no hub flicker).

## G34 — Wedges drew as dots: the colour-quad `text` NBT was not JSON · 2026-07-24 (🟡 built, pending in-game)

Owner tested the v2 wheel and reported the 50 wedges rendered as scattered coloured **dots**, not filled pie slices.

**First guess was wrong and reverted.** The initial read was that `text_opacity: 0` suppressed the whole display. Disassembling the real 1.21.1 `DisplayEntityRenderer$TextDisplayEntityRenderer` (from this project's own loom-cached named jar) disproves it: the background quad is gated on `iload 13 / ifeq` — that is `background != 0` — and the text-opacity local is only ever read for the glyph draw. Opacity restored to `0`.

**Actual root cause.** `DisplayEntity$TextDisplayEntity.readCustomDataFromNbt` reads the `text` field with `getString("text")` and hands it to `Text$Serialization.fromJson(...)` — the field is a **raw JSON text component**, not plain text. `WheelDisplayVisual.applyQuadColor` wrote a bare `" "`, which is not valid JSON, so parsing threw. The bytecode's exception table catches it, logs `Failed to parse display entity text {}`, and — critically — **never calls `setText`**, so the entity kept an empty text.

**Why an empty text becomes a dot.** The renderer sizes the background quad from the split lines: corners are `(-1,-1) (-1,h) (w,h) (w,-1)` with `w = TextLines.width()` and `h = lines * 10`, all under a `scale(-0.025)`. Empty text ⇒ `w = 0`, no lines ⇒ a **1×1 px** quad. The wedge transform then scaled that speck by `len/BG_W` × `wid/BG_H` instead of scaling a 5×11 px quad — a few centi-blocks across. 600 of them = the dotted radial pattern in the screenshot. Fixed by writing the JSON literal `"\" \""`.

**Bonus — `BG_W`/`BG_H`/`BG_CX`/`BG_CY` are now verified, not estimated.** Re-derived from the same bytecode: line height is `9 + 1 = 10` px and the renderer applies `translate(1 - w/2, -h, 0)`. For a one-space text (`w = 4`, 1 line) that gives exactly 0.125 × 0.275 blocks with centre offsets 0.0125 / 0.1375 — the four constants already in the file. This was the one estimated number in the group; it is now exact.

**Files:** `wheel/WheelDisplayVisual.java` (`applyQuadColor` text field + `BG_*` doc note). Docs: TG34 verdict + new row A8 (a wedge must be a solid band, not dots), this log.

**Next:** owner re-places the wheel and re-checks TG34 §A.

## G09 — Backup engine rebuilt: full-tree deduped snapshot · kind-based retention · verify · granular · cloud pull BUILT · 2026-07-23 (🟡 built, pending in-game)

Owner asked to fix the unbuilt/broken items in TG9 (new naming scheme, §F cloud, §G data paths, and more) before testing. Deep-searched the whole G09 code first; found two silent bugs and a real data-loss hole, then rebuilt the backup engine in tested slices. **All build-green (full `gradlew build` incl. monolith gate + verify tasks), NOT in-game tested.**

**Two silent bugs the old code had (now fixed).** Retention keyed off the folder NAME (`name.startsWith("auto-")`): (1) a manual `/cb backup save auto_x` would be classified auto and silently pruned; (2) `pre-restore-*` safety copies were written with `auto=true` but matched neither prune rule, so they accumulated forever. Fix: a backup's **kind** (`manual`/`auto`/`safety`) now lives in `manifest.json`, and retention prunes by kind — AUTO by count + optional disk budget, SAFETY by count (never below 1). Reserved name prefixes block a hand-typed name from impersonating a generated one.

**The §G data-loss hole.** The old snapshot copied only `slots.json`, `config.json`, `textures/`, `sources/`; restore MOVED only those aside, so a restore left notes/locks/categories/markers/data stale — a mixed state, not a snapshot. Now a snapshot captures the **whole `config/customblocks` tree** (except `backups/`, `updates/`, the pack zip), and restore reconstructs live to EXACTLY the backup's data set (whole-tree move-aside safety copy first; rolls back on failure).

**Dedup pool (P2).** File bytes are content-addressed into `backups/_pool/<sha>` and each backup stores a path→sha list (format v3). A second auto-backup that re-references unchanged textures costs ~nothing, so far more restore points fit on disk; deleting a backup GCs unreferenced blobs. Verified the pool algorithm end-to-end with a standalone JDK simulation (dedup hit, exclusions, restore fidelity, GC keep-set — all pass) and the SHA-256 hex against a known vector.

**Plus:** `CbPaths` central path constants; `/cb backup verify [name]` (deep hash check) + a boot guard that refuses to overwrite good backups when live `slots.json` is missing/corrupt; wall-clock daily schedule + disk-budget retention + a pre-update backup on jar-version change + `pre-setall`/`pre-retextureall`/`pre-tempretexture` safety copies; `/cb backup preview` (diff), `/cb backup contents` (browse), `/cb backup load <name> <entry>` (granular single-store restore); cloud **upload was already built** — added `/cb backup pull <code>` and fixed the worker to remember the backup's name (`/backup/<code>/name`).

**Refactor (§9.3 no-monolith gate).** `BackupManager` (987 lines) split into focused siblings: `BackupPool`, `BackupRestore`, `BackupArchive`, `BackupIntegrity`, `BackupView`, `BackupRetention`, plus `CbPaths`; `BackupCommands` (534) split with `BackupScreenCommands`. The restore algorithm was relocated unchanged (Sim-verified before the move).

**Files:** new `core/CbPaths`, `core/BackupPool`, `core/BackupRestore`, `core/BackupArchive`, `core/BackupIntegrity`, `core/BackupView`, `core/BackupRetention`, `command/handlers/BackupScreenCommands`; rewrote `core/BackupManager`, `core/AutoBackup`, `command/handlers/BackupCommands`; touched `CustomBlocksConfig(+Store)`, `CustomBlocksMod`, `PayloadRegistrar`, `SetAll/RetextureAll/TempRetexture` (safety kind), `cloud/CloudVaultClient`, `cloudflare/worker.js`. Docs: TG9 rewritten (A–J, all 🎯, 0% pending retest), GROUP_09 (decisions + technical contract + data layout corrected — trash IS its own store; snapshot IS whole-tree), this log.

**NOT done in this pass:** **§D/§E Screens (P7)** — moving the Backup/Trash surfaces fully into **G27** (multi-select delete, read-only block browse, per-block restore, retiring the chest menus) is visual Screen work and needs a dedicated in-game session; command/data behavior for both is complete. **Owner action before §F test:** redeploy `cloudflare/worker.js`.

**Next:** owner tests TG9 §A–§J in-game; then P7 Screen migration in G27.
