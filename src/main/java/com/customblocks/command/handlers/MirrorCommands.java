 ...
 * (374/400 lines — one more block would bust the monolithGate gate; a domain-split handler is the §5
...
public final class MirrorCommands {
    ...
    private static int status(CommandContext<ServerCommandSource> ctx) {
        ...
        Chat.info(ctx.getSource(), "Named-texture mirror: " + (on ? CbFmt.OK + "ON" : CbFmt.BAD + "OFF")
                + " " + CbFmt.DIM + "· " + CbFmt.BODY + TextureNameMirror.fileCount() + CbFmt.DIM + " file(s)"
                + " " + CbFmt.FAINT + "(/cb config mirrornames on · off · rebuild)");
        Chat.info(ctx.getSource(), CbFmt.DIM + "Folder: " + CbFmt.BODY + TextureNameMirror.folderPath());
        return 1;
    }
    ...
    private static int on(CommandContext<ServerCommandSource> ctx) {
        ...
        Chat.success(ctx.getSource(), "Named-texture mirror " + CbFmt.OK + "on" + CbFmt.RESET + " — wrote " + CbFmt.BODY + files
                + CbFmt.RESET + " file(s) to textures_names/. Browse your blocks by name there. "
                + CbFmt.DIM + "(slot_N.png stays the real texture — this is just a readable copy.)");
        return 1;
    }
    ...
    private static int off(CommandContext<ServerCommandSource> ctx) {
        ...
        CustomBlocksConfig.save();
        Chat.success(ctx.getSource(), "Named-texture mirror " + CbFmt.BAD + "off" + CbFmt.RESET + ". Existing files are left in place; "
                + "no new ones are written. " + CbFmt.DIM + "(/cb config mirrornames on re-enables and refreshes them.)");
        return 1;
    }
    ...
    private static int rebuild(CommandContext<ServerCommandSource> ctx) {
        ...
        Chat.success(ctx.getSource(), "Rebuilt the named-texture mirror from scratch — " + CbFmt.BODY + files + CbFmt.RESET + " file(s)"
                + (CustomBlocksConfig.mirrorNamedTextures ? "."
                        : " " + CbFmt.DIM + "(note: the mirror is currently OFF, so it won't auto-update until you turn it on)."));
        return 1;
    }
}
