...
> ✅ **All sections confirmed in-game.** Kept for re-test reference.
...
|---|---|
| **Verdict** | 🟢 All pass |
| **Progress** | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 · 27 / 27 passed |
| **Last tested** | 2026-06-15 (latest) |
| **Jar** | 1.0.0 |
| **Tester** | — |

---

## What changed in this redesign (read first)

- **`/cb colors` is now `/cb coloring`** (old name still works). It's a polished hub for everything; there's also a **Coloring** tile on the `/cb` dashboard.
- **`/cb bgstudio` with no id opens a block picker** — pick a block, then the studio. `/cb bgstudio <id>` still goes straight in.
- **`/cb tolerance <value> [id]`** — value comes first now. **No id = global default.** **With an id = just that block** (saved per-block; global stays). e.g. `/cb tolerance 40` vs `/cb tolerance 40 myblock`.
- **Palette is now useful** — its swatches appear as one-click quick-picks in the BgStudio fill picker, the Gradient endpoints, and Custom Colour.
- **`/cb gradient` with no args opens the Gradient Builder GUI** (was an error before).
...
| | What | § |
|:--:|---|:--:|
| ✅ | **`/cb resize`** · **`/cb exportpng`** (with **[download]** link) | §1 |
...
# ✅ Confirmed — re-check only if something feels off

## §1 · Resize + Export PNG  *(confirmed)*
...
- **① Resize up** — `/cb resize g10a 256`
  - ✅ **Pass:** chat says the block texture was resized to 256×256. The block looks sharper.
- **② Resize down** — `/cb resize g10a 64`
  - ✅ **Pass:** resizes to 64×64 (chunkier). No errors.
- **③ Refuses a textureless / locked block** — `/cb create g10empty` (no URL), then `/cb resize g10empty 128`; also `/cb lock g10a` → `/cb resize g10a 128`.
  - ✅ **Pass:** clear messages ("no texture to resize" / "is locked") — no crash.
- **④ Export PNG** — `/cb exportpng g10a`
  - ✅ **Pass:** chat gives the export path + a clickable **[download]** link.
...
> endpoints. The **Gradient Builder** GUI lets you pick the two colours visually (type a hex, pick from
> a block, or grab from your palette) and preview the swatches before creating them.

> 🧰 **Before you start:** two URL-made blocks, e.g. `/cb create g10a <url1>` and `/cb create g10b <url2>` with different-coloured images.

**Try these:**

- **① Gradient command** — `/cb gradient g10a g10b 4`
  - ✅ **Pass:** chat says `Created 4 gradient block(s): gradient_1, gradient_2, gradient_3, gradient_4.` Place them in a row — each is a smooth colour step.
- **② Gradient is one undo** — `/cb undo`
  - ✅ **Pass:** all four `gradient_*` blocks vanish in a single undo.
- **③ Gradient Builder GUI** — `/cb gradient` (no args) *(or `/cb coloring` → Gradient Builder)*
  - ✅ **Pass:** a 5-row chest opens with Colour A / Colour B tiles, a Steps ±, preview swatches, palette quick-pick swatches, and a green **Create gradient** button.
- **④ Pick colours in the GUI** — Left-click "Colour A" → type `red` in the anvil. Left-click "Colour B" → type `#0000FF`. Set steps to 3.
  - ✅ **Pass:** preview swatches appear between A and B. The Create button shows `3 block(s) from #FF0000 → #0000FF`.
- **⑤ Pick from a block** — Right-click "Colour A" → chat prompts `/cb gradientpick a <blockId>` → type `g10a`. The GUI reopens with Colour A set to g10a's average colour.
...
> 🧰 **Before you start:** a URL-made block with a clear subject on a plain-ish background, e.g. `/cb create g10a <image-url>`.
...
- **① Open it** — `/cb bgstudio g10a` *(or `/cb editor g10a` → "Background Studio", or the Coloring hub)*
  - ✅ **Pass:** a 5-row chest opens with four mode tiles, a strength −/＋, a **fill colour** tile (black, "#000000"), palette quick-fill swatches, an **↩ Undo last change** tile, and a green **Apply**.
- **② Background only** — click "Background only", set strength ~30%, click **Apply**.
  - ✅ **Pass:** the flat background is replaced with black. `/cb undo` restores.
- **③ Change fill colour** — Left-click the fill-colour tile → type `red` (or `#FF0000`) in the anvil.
  - ✅ **Pass:** the tile updates to red. Apply again → background is now RED.
- **④ Right-click resets fill** — Right-click the fill-colour tile.
  - ✅ **Pass:** resets to black.
- **⑤ Smart** — click "Smart", **Apply**.
  - ✅ **Pass:** only the main subject survives; loose background clutter is gone. Undoable.
- **⑥ `/cb tolerance` (value first now)** — global: `/cb tolerance 60` → chat confirms the **global** default changed. Per-block: `/cb tolerance 60 g10a` → re-applies g10a at 60% immediately and saves it as g10a's own strength; chat notes the global default stays.
...
- **① Open** — `/cb editor g10a` → **Colour Variants**.
  - ✅ **Pass:** a row of swatches (Lighter, Darker, Vivid, Muted, Complementary, two Split-Complements), each showing the rough resulting colour.
- **② Make one** — click **Lighter**.
  - ✅ **Pass:** chat `"g10a_lighter" created`; you're handed the new block; it's a lighter version of g10a. `/cb undo` removes it.
...
> 💡 **What changed:** the Palette is now a **shared colour source**. Its working swatches appear as one-click quick-picks in the **Background Studio fill** picker, the **Gradient Builder** endpoints, and **Custom Colour**.

- **⓪ Shared source** — add a couple of colours (e.g. `red`, `#00AAFF`). Then open `/cb bgstudio <id>`, `/cb gradient`, and `/cb coloring` → Custom Colour.
  - ✅ **Pass:** those same colours show up as clickable swatches in all three.

- **① Add via anvil** — `/cb colors` → **Palette** → click **＋ Add colour** → type `red` in the anvil → take the output.
  - ✅ **Pass:** the palette reopens with a red swatch in your working set.
- **② Add via hex** — click **＋ Add colour** → type `#00AAFF` → take it.
  - ✅ **Pass:** `#00AAFF` appears as a light-blue swatch.
- **③ Remove swatch** — Left-click the red swatch.
  - ✅ **Pass:** red is removed from the working set.
- **④ Save + load** — click **Save current as…** → type `mytheme` → take it. Then click **Clear working set**. Load it back by left-clicking `mytheme` in the saved list.
...
- **⑥ Coloring hub layout** — `/cb coloring` *(or `/cb colors`, or the dashboard's Coloring tile)*
  - ✅ **Pass:** titled "Coloring"; shows Background Studio, Palette, Gradient Builder, Custom Colour (top row) and Colour Variants, Live Recolour, Screen Eyedrop (lower row).
...
- **① Live recolour** — `/cb editor g10a` → **Live Recolour** *(or `/cb livecolor g10a`)*.
  - ✅ **Pass:** a screen opens showing a preview of g10a's texture with three drag bars (Hue / Saturation / Lightness). **Dragging updates the preview live.** Click **Apply** → the real block recolours in-world. `/cb undo` reverts.
- **② Screen eyedrop** — `/cb eyedrop` *(or Colours hub → Screen Eyedrop)*.
  - ✅ **Pass:** the world stays visible with a crosshair; click any pixel → your chat opens pre-filled with `/cb palette add #RRGGBB` for that pixel's colour.
