...
| `favorite`/`fav`/`unfavorite` + `recent` | **Feature owned by G25**; G17 keeps regression checks only | G17, G25 |
...
| Chat download links | **REMOVE IP/host from every chat download button** (public mod must not leak server address); current `http://<host>:<port>/export/…` links also fail (connection refused). Rethink delivery. Confirmed call sites: `ResourcePackServer.getPngUrl()` (G10.5 `exportpng`, G12 PNG), `getZipUrl()` (G11.6 `category export`, G12 ZIP). | G12 (.2–.5), G10 (.5), G11 (.6), anywhere a `[download]` link is posted |
...
| G17 | Regressions | cross-cutting regression checks only (undo/redo/give args/search/delete#). favorite/recent **checks only** — feature→G25 |
| G18 | Notes | `note` rework only — **staging system fully REMOVED** |
| G19 | Display | faces, `hologram`, `text`, `givedisplayblock`, badges, `showcase`*. ~~particles~~→G16 |
...
- ⚠️ **`showcase`** — appears in G19 (display) and G23 (player experience). Tentative: G23 owns the player-facing `show`/`showcase`; G19 owns display-block rendering. Confirm when either is built.
...
| G11 | ✅ | ✅ | scattered category cmds unified → `/cb category <action>` (confirmed in code); 15✅; G11.6 export link host-leaks (§A); `bulkretexture` tile = dead (never built); follow-ups logged |
...
| G18 | — | 📋 | **staging system fully SCRAPPED (doc cleaned 2026-06-21)**; G18 = notes only (G18.1–.7) |
| G19 | — | 📋 | not built. **Ownership sweep 2026-06-21: particles→G16; showcase tentative vs G23** |
| G20 | — | — | not built |
| G21 | — | — | not built |
| G22 | — | — | not built |
