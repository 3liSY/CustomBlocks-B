...
| --- | --- | --- | --- |
| D | Recycle-bin deletion system and no-rejoin refresh | Done ✅ | Regressed 💔 |
...
## D2 - Restoring from the trash keeps the original picture - Regressed 💔

| | |
| --- | --- |
| **Check** | A block brought back from the trash comes back whole, including the picture it was made from. |
| **Pass rule** | A restored block can be edited exactly like one that was never deleted. |
| **Pass mark** | ✅ `YYYY-MM-DD` |

| # | Do this | Should happen | SP | MP |
| --- | --- | --- | --- | --- |
| D2a | Make a block from a link, delete it, open `/cb trash`, press Restore, then type `/cb setbg <id> purple`. | It turns purple. It must NOT say "no stored picture to re-bake". | 🎯 | 🎯 |
| D2b | On that same restored block, type `/cb resize <id> 256`. | Resizes cleanly from the original picture. | 🎯 | 🎯 |
| D2c | On that same restored block, type `/cb variants <id>`. | The colour family builds from the original picture. | 🎯 | 🎯 |
| D2d | Make another block from a link, delete it, then `/cb undo`, then `/cb setbg <id> purple`. | It turns purple. This is the path that was losing the picture. | 🎯 | 🎯 |
