...
> **Display per [ADR-005](../adr/README.md#adr-005), NOT the old
...
> ⚠️ **SUPERSEDED 2026-07-09/10:** this pass predates the UI medium audit, which ruled the OPPOSITE
> direction — `ArabicBrowserScreen` is KEPT and merges into one unified Arabic Screen (see
> `GROUP_13_ARABIC.md`'s "UI medium audit (2026-07-09)" note and Test G13.10). Do NOT replace the screen
> with a chest GUI. Section kept for history only.
...
- Test: **G13.10** (now targets the unified Arabic Screen, not a chest GUI — see `GROUP_13_ARABIC.md`).
