...
## G05-6 · Live in-place texture swap — kill the full reload on a pixel-only edit ⭐ (the "reload freezes every edit" pain) · **DESIGN — awaiting ✅ to build**
...
| Reload state | ⏳ DESIGN — awaiting owner ✅ (no code yet) |
...
## G05-7 · Skip the redundant first-join reload (singleplayer/integrated) — persist the applied hash · **DESIGN — awaiting ✅ to build**
...
### Chosen design (NOT built) — persist the applied hash next to the loose pack

- On a successful apply (`applyReload` completion, `:138`), write the applied hash to a small sidecar file,
  e.g. `resourcepacks/CustomBlocks/.cbpackhash` (one line). Also set `lastAppliedHash` as today.
- At the top of `regenerate`, if `lastAppliedHash` is null, **seed it from that sidecar file** (once per
  launch). Then the existing `hash.equals(lastAppliedHash)` guard matches on the first open of a session and
  the reload is skipped — MC already has the pack from boot.
- Extra safety gate: only skip when the sidecar hash matches AND the loose pack folder is non-empty / the
  `pack.mcmeta` exists (so a wiped or half-written pack still forces a real reload — nothing lost).
...
| Fix | persist applied hash to `resourcepacks/CustomBlocks/.cbpackhash`; seed `lastAppliedHash` from it at launch |
| Out of scope | the vanilla boot reload (all packs, MC startup) — not removable |
| Reload state | ⏳ DESIGN — awaiting owner ✅ (no code yet) |
